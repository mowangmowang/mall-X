...
/**
 * CORS 预检 (Preflight) 集成测试 - mall-search 版
 * <p>
 * 验证目标：mall-search 不依赖 mall-security，跨域通过 {@code CorsFilterRegistration}
 * 显式注册的 {@code CorsFilter}（order=HIGHEST_PRECEDENCE）处理。预检 OPTIONS 请求
 * 必须在 Spring MVC DispatcherServlet 之前被拦截，并返回正确的 CORS 响应头。
 * </p>
 * <p>
 * 该测试回归以下任一情况即视为不通过：
 * <ul>
...
class CorsPreflightTest {
    ...
    void preflight_authorizationHeader_shouldBeAllowed() throws Exception {
        mockMvc.perform(options("/esProduct/search")
