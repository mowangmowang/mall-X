...
| 6 | ✅ | 移除 MyBatis 依赖，ReturnReason 改用 OpenFeign（-4 依赖） |
| 7 | ✅ | 新增 SSE 流式输出端点 `/ai/product/qa/stream` |
...
Stage 7 成果：新增 SSE 流式输出端点 `POST /ai/product/qa/stream`（`text/event-stream`）。Spring AI `chatClient.stream().content()` 返回 `Flux<String>`，前端用 `fetch + ReadableStream` 消费即可实现"打字机"效果。旧同步端点 100% 向后兼容。退货建议的流式版（边解析 JSON 边输出 guideQuestion）复杂度高，留 Stage 8 协议升级时一并处理。
