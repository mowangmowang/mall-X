<template>
		<view class="content">
			...
			<!-- 价格区间输入 -->
			<view class="price-range" :class="{show: filterIndex === 2}">
				<input class="price-input" type="digit" v-model="searchParam.startPrice" placeholder="最低价" @confirm="onPriceRangeChange" @blur="onPriceRangeChange" />
				<text class="price-sep">—</text>
				<input class="price-input" type="digit" v-model="searchParam.endPrice" placeholder="最高价" @confirm="onPriceRangeChange" @blur="onPriceRangeChange" />
	...
	<script>
		...
		export default {
			...
			onLoad(options) {
				...
				this.headerTop = pageHead ? pageHead.offsetHeight + 'px' : '0px';
				// #endif
				if (options.sid) {
				...
				if (options.keyword) {
					...
					uni.setNavigationBarTitle({ title: this.searchParam.keyword });
				}
				if (options.startPrice) {
					this.searchParam.startPrice = options.startPrice;
				}
				if (options.endPrice) {
					this.searchParam.endPrice = options.endPrice;
				}
			...
			methods: {
				...
				//价格区间变更
				onPriceRangeChange() {
					if (this.searchParam.startPrice === '' || this.searchParam.startPrice === null) {
						this.searchParam.startPrice = null;
					}
					if (this.searchParam.endPrice === '' || this.searchParam.endPrice === null) {
						this.searchParam.endPrice = null;
					}
