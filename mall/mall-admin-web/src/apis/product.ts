 ...
 * 根据商品ID查询审核记录
 */
export function getProductVertifyRecordAPI(productId: number) {
  return http({
    url: '/product/vertifyRecord/' + productId,
    method: 'get',
  })
}
 ...
 * 批量修改审核状态
 */
export function productUpdateVerifyStatusAPI(params: { ids: string; verifyStatus: number; detail: string }) {
  return http({
